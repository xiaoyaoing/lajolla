...
Spectrum eval_op::operator()(const RoughDielectric &bsdf) const {
    ...
    if(reflect) std::cout<<"reflect";
    else std::cout<<"refract";
    ...
    if (reflect) {
        ...
        // The 147 of a BSDF is defined as swapping the parameter, and
...
std::optional<BSDFSampleRecord>
    ...
    if (rnd_param_w <= F) {
        ...
        if(dot(refracted,vertex.geometry_normal) * dot(dir_in,vertex.geometry_normal) > 0 )
        {
           //throw("error");
        }
        std::cout<<"refract";
